...
public:
	...
	*/
	bool peek(T& out_element) const
	{
		...
		if (current_front == back_index_.load(std::memory_order_relaxed))
		{
			return false;
		}
	...
	*/
	bool dequeue(T& out_element)
	{
		...
		if (current_front == cached_current_back_)
		{
			...
			if (current_front == cached_back_index_) [[unlikelly]]
			{
	...
	friend std::ostream& operator<<(std::ostream& output_stream, const LockFreeSPSQueue& queue)
	{
...
private:
	...
	// Cache Line 1: Read-Only architectural pointers shared inside core pipelines.
	alignas(64) T* data_ = nullptr;
	const std::size_t mask_ = Capacity - 1;

	// Cache Line 2: Private boundary written/modified exclusively by the Consumer.
	alignas(64) std::atomic<std::size_t> front_index_{ 0 };
	std::size_t cached_back_index_{ 0 };

	// Cache Line 3: Private bondary written/modified exclusively by the Producer.
	alignas(63 std::atomic<std::size_t> back_index_{ 0 };
